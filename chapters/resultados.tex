 ...
 \label{subsec:paso1a4}
	...
	\label{subsec:pfp}
 	...
 	\begin{enumerate}
 		...
 		\item \textbf{Creaci\'{o}n de la caja y solvataci\'{o}n:} La prote\'{i}na se sumergi\'{o} en una caja de simulaci\'{o}n tridimensional en presencia de mol\'{e}culas de agua (generalmente modelos como  \textit{TIP3P} o \textit{SPC/E}), recreando as\'{i} un entorno fisiol\'{o}gico acuoso.
 		...
 		\item \textbf{Minimizaci\'{o}n de la energ\'{i}a del sistema:} Se utiliz\'{o} un algoritmo para relajar la estructura y eliminar conflictos est\'{e}ricos o tensiones geom\'{e}tricas artificiales, llevando el sistema a un m\'{i}nimo energ\'{e}tico local y previniendo inestabilidades num\'{e}ricas.
 		
 		\item \textbf{Equilibrio del sistema:} Esta fase consta normalmente de dos etapas: primero, un equilibrio en el ensamble NVT (n\'{u}mero de part\'{i}culas, volumen y temperatura constantes), tambi\'{e}n conocido como \textit{ensamble can\'{o}nico}. En este ensamble, la temperatura del sistema se acopla a un termostato externo (como el Berendsen o \textit{Velocity-Rescaling}) para mantenerla en el valor deseado (e.g., 300 K), permitiendo que las velocidades de las part\'{i}culas se ajusten correctamente. Posteriormente, continua la segunda etapa en el ensamble NPT (n\'{u}mero de part\'{i}culas, presi\'{o}n y temperatura constantes) para ajustar la densidad del disolvente a condiciones ambientales.
 		
 		\item \textbf{Simulaci\'{o}n de producci\'{o}n:} Una vez equilibrado, el sistema se simuló en el ensamble NPT o NVT durante un tiempo extenso (del orden de nanosegundos a microsegundos en tiempo real). Esta fase genera la trayectoria principal que se analizó para extraer propiedades termodin\'{a}micas, din\'{a}micas y estructurales.
	...
	La comparaci\'{o}n presentada en la Figura~\ref{fig:Tubs} se refiere a dos variantes estructurales: (1) Una prote\'{i}na nativa con 6,812 \'{a}tomos (estructura de referencia) y (2) una variante mutada con 6,816 \'{a}tomos (reemplazo de un residuo aminoac\'{i}dico). Además, esta variación estructural proteica tiene una relevancia médica debido a que este cambio sutil, provoca una enfermedad neurodegenerativa.
	
	Los resultados obtenidos mostraron diferencias en las pendientes de la regresi\'{o}n lineal analizadas espec\'{i}ficamente en la etapa final del procesamiento (despu\'{e}s de una din\'{a}mica molecular de 10 ns). Esta diferencia estructural se observa a pesar de la alta semejanza global entre ambas variantes.
